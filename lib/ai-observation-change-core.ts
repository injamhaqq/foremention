...
const exactAnswerKey = (answer: AiObservationAnswer) => [
  answer.promptKey,
  cleanText(answer.promptText || ""),
  answer.provider,
  answer.model || "model-not-recorded",
].join("\u0000");
const questionLabel = (answer: AiObservationAnswer) => cleanText(answer.promptText || "") || answer.promptKey;
...
export function buildAiObservationChangeGraph(input: Input): AiObservationChangeGraph {
  ...
  if (!sameMethodology) {
    return {
      ...
      comparable: false,
      events: [],
      note: "The methodology version changed. The collections remain valid observations on their own, but Foremention will not label their difference as movement.",
    };
  }

  const exactAssessment = assessExactQuestionComparability(
    input.latest.id,
    input.previous.id,
  ...
  if (!exactAssessment.comparable) {
    return {
      ...base,
      status: "withheld",
      comparable: false,
      events: [],
      note: `${exactAssessment.reason || "The exact reviewed answer matrix changed."} The collections remain valid observations on their own, but Foremention will not label their difference as movement.`,
    };
  }

  const latestByKey = new Map(latestAnswers.map((answer) => [exactAnswerKey(answer), answer]));
  const previousByKey = new Map(previousAnswers.map((answer) => [exactAnswerKey(answer), answer]));
  ...
  for (const [key, latestAnswer] of latestByKey) {
    ...
    if (previousAnswer.brandPresent !== true && latestAnswer.brandPresent === true) brandGains.push(questionLabel(latestAnswer));
    if (previousAnswer.brandPresent === true && latestAnswer.brandPresent !== true) brandLosses.push(questionLabel(latestAnswer));
  ...
  if (citationGains || citationLosses) events.push({
    ...
    detail: "Citation movement compares canonical URL observations inside the same exact persisted buyer-question/provider/exact-model answer slots.",
  });
  ...
  return {
    ...
    summary,
    events,
    note: "These events use the exact comparable reviewed collection pair selected by Foremention. Observed differences do not establish causation.",
  };
}
