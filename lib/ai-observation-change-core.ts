...
const answerKey = (answer: AiObservationAnswer) => `${answer.promptKey}\u0000${answer.provider}\u0000${answer.model || "model-not-recorded"}`;
...
export function buildAiObservationChangeGraph(input: Input): AiObservationChangeGraph {
  ...
  const missingExactModels = [...latestAnswers, ...previousAnswers].some((answer) => !answer.model);
  ...
  if (!sameMethodology || missingExactModels || !sameMatrix) {
    const reason = !sameMethodology
      ? "The methodology version changed."
      : missingExactModels
        ? "At least one reviewed answer is missing exact model provenance."
        : "The reviewed buyer-question/provider/exact-model matrix changed.";
    return {
      ...base,
      status: "withheld",
      comparable: false,
  ...
  return {
    ...
    summary,
    events,
    note: "These events use the exact comparable reviewed collection pair already selected by Foremention. Observed differences do not establish causation.",
  };
}
