import { browserRunConfigured, retrieveFreeWebEvidence } from "@/lib/free-web-retrieval";
import {
...
export function cloudflareAiConfigured() {
  return Boolean(runtime.__FOREMENTION_CLOUDFLARE_AI__ && browserRunConfigured() && process.env.CLOUDFLARE_MODEL);
}
...
function selectedCitations(answer: string, available: ProviderCitation[]) {
  ...
  if (!marker) throw new ProviderRequestError("Cloudflare Workers AI + Browser Run", 502, "The grounded answer did not identify which retrieved sources supported it.");
  ...
  if (!unique.length) throw new ProviderRequestError("Cloudflare Workers AI + Browser Run", 502, "The grounded answer selected no valid retrieved source.");

  const cleanAnswer = answer.replace(/(?:^|\n)\s*SOURCES:\s*[^\n\r]+/i, "").trim();
  if (!cleanAnswer) throw new ProviderRequestError("Cloudflare Workers AI + Browser Run", 502, "The grounded answer returned no answer text.");
...
export const cloudflareAdapter: AnswerProviderAdapter = {
  ...
  async run(prompt: ProviderPrompt, options): Promise<ProviderAnswer> {
    ...
    try {
      ...
      throw new ProviderRequestError("Cloudflare Workers AI + Browser Run", 502, detail);
    }
  },
};
