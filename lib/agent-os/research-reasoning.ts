...
const stringArray = (value: unknown, maxItems: number, maxChars: number) =>
  Array.isArray(value) ? value.filter((item): item is string => typeof item === "string").slice(0, maxItems).map((item) => clean(item, maxChars)).filter(Boolean) : [];

export function validateResearchInsightReasoningOutput(value: unknown, allowedEvidenceKeys: Set<string>) {
  ...
  } satisfies ResearchInsightReasoningOutput;
}
