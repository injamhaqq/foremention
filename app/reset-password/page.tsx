...
export default async function ResetPasswordPage() {
  ...
  return <main className="auth-page auth-page--evidence"><div className="auth-brand"><Wordmark /><div className="auth-brand__positioning"><span className="auth-brand__kicker">Recommendation intelligence for B2B SaaS</span><strong>Restore access. Preserve evidence history.</strong><p>Your verified recovery session changes account access only. Existing recommendation records and workspace evidence remain unchanged.</p><ol className="auth-brand__trace" aria-label="Recovery boundary"><li><span>01</span><small>Verified</small></li><li><span>02</span><small>Password</small></li><li><span>03</span><small>Session</small></li><li><span>04</span><small>Workspace</small></li></ol><span className="auth-brand__principle">EVIDENCE BEFORE THEATRE</span></div><Link href="/">← Back to site</Link></div><section className="auth-card" data-evidence-record="auth"><span className="auth-record-index">AUTH / PASSWORD</span><span className="eyebrow">Account recovery</span><h1>Choose a new password.</h1><p>Your recovery link has verified this browser. Set a new password, then we will open your workspace.</p><SetPasswordForm /></section></main>;
}
