...
export default async function SharedRecordPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ view?: string | string[] }> }) {
  ...
  return <main className="shared-record-shell">
    ...
    <section className="shared-record-hero">
      <div><span className="eyebrow">Recommendation Record · {run.id.slice(0, 8).toUpperCase()}</span><h1>{viewMode === "executive" ? "Decision context without workspace exposure." : "Evidence that can be challenged."}</h1><p>This shared view is frozen to one recorded collection. It does not expose workspace navigation, private notes, credentials, or unrelated customer data.</p></div>
    ...
    {viewMode === "executive" ? <section className="shared-record-list"><article><div className="shared-record-list__meta"><span>Executive view</span><span>{safeConclusion ? "Reviewed conclusion available" : "Conclusion withheld"}</span></div><h2>What this Record can support</h2><p>{safeConclusion ? `${reviewed} reviewed answer${reviewed === 1 ? "" : "s"} sit inside an observed collection of ${run.answer_count}. Use the stakeholder view to inspect the underlying answer and returned-reference evidence before making a stronger claim.` : "The current review state is not strong enough to present a safe conclusion. The Record remains useful as an observed collection, but stronger interpretation should stay withheld."}</p><p className="table-caption">Executive view intentionally withholds detailed answer text and returned source locations. Switch to stakeholder view when evidence inspection is required.</p></article></section> : <section className="shared-record-list">
