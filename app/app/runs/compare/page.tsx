...
export default async function RunComparisonPage({ searchParams }: { searchParams: Promise<{ left?: string; right?: string }> }) {
  ...
  return <main className="workspace">
    <div className="workspace-heading"><div><span className="eyebrow">Exact run comparison</span><h1>Observed differences across a proven comparable pair.</h1><p>Both collections passed human review and use the same methodology plus the exact persisted buyer-question text, provider, and model matrix. Differences are observations, not proof of causation or buyer behavior.</p></div><Link className="button button--outline" href="/app/runs">Choose other runs</Link></div>
