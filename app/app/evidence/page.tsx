...
import { getPrimaryWorkspaceRole, loadEvidence, loadLatestReviewedAnswers } from "@/lib/data";

export default async function EvidencePage() {
  const viewer = await requireViewer("/app/evidence");
  const [evidence, collected, role] = await Promise.all([loadEvidence(viewer), loadLatestReviewedAnswers(viewer, 12), getPrimaryWorkspaceRole(viewer)]);
