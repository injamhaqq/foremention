...
export default async function AnalyticsPage() {
  ...
  const comparisonWindowNote = previous
    ...
    : reviewedRunCount > 1
      ? "Other reviewed collections exist, but none matched the current question + provider + exact model + methodology signature in the comparison window"
  ...
  return <main className="workspace" data-product-state={state}>
    ...
    <section className="panel panel--flush ai-observation-events">
      <div className="panel-heading panel-heading--padded"><div><span className="eyebrow">Change Graph · AI observations</span><h2>{intelligence.telemetry === "fictional" ? "Fictional demo movement is not presented as customer evidence." : previous ? observationEvents.length ? `${observationEvents.length} supported change event${observationEvents.length === 1 ? "" : "s"} in the exact comparison pair.` : "No supported AI-observation differences in the exact comparison pair." : "One reviewed collection establishes the AI baseline."}</h2><p>{intelligence.telemetry === "fictional" ? "Use a real workspace collection for evidence-backed change events." : previous ? "Foremention first selects the prior run using the same question set, provider, exact model, and methodology. Only then does this layer explain supported differences in brand presence, returned citations, unique cited sources, and exact answer text." : "A second reviewed collection with the same question set, provider, exact model, and methodology is required before Foremention reports AI-observation movement."}</p></div>{previous && <Link className="text-link" href={`/app/runs/${latest.id}`}>Inspect current AI Results →</Link>}</div>
      {observationEvents.length ? <div className="notification-list">{observationEvents.map((event) => <article key={event.id}><div><span>{observationKind(event.kind)} · {event.direction}</span><strong>{event.title}</strong><p>{event.detail}</p></div><Link href={event.href}>Inspect →</Link></article>)}</div> : <div className="empty-state empty-state--border"><h2>{previous ? "Nothing is promoted into an AI change event without a supported difference." : "No comparable AI movement yet."}</h2><p>{previous ? "The selected comparable pair matched across the supported observation types, or only unsupported differences occurred." : "Other reviewed collections can remain valid evidence on their own even when they do not qualify as a comparable prior run."}</p></div>}
