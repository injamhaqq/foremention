...
export default async function AnalyticsPage() {
  ...
  return <main className="workspace" data-product-state={state}>
    <div className="workspace-heading"><div><span className="eyebrow">Did anything change?</span><h1>Analytics</h1><p>Compare reviewed collections, question evidence yield, source-review progress, and saved cited-page changes. Trend deltas appear only when Foremention finds the same question set, provider, exact model, and methodology. Movement is observed change, not proof that an action caused it.</p><p className="table-caption"><strong>{productStateLabel(state)}</strong> · Latest reviewed baseline {latest.date} · {latest.answers} answer{latest.answers === 1 ? "" : "s"} · {comparisonWindowNote}</p></div><Link className="button button--outline" href="/app/runs">Inspect AI Results</Link></div>
