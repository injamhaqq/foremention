...
import { getPrimaryWorkspaceRole, loadSourceEvidenceContexts } from "@/lib/data";
...
export default async function SourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sources, role] = await Promise.all([loadTruthfulSourceMap(viewer), getPrimaryWorkspaceRole(viewer)]);
  ...
  return <main className="workspace">
    ...
    <div className="record-grid">
      <section className="panel panel--wide"><div className="record-url"><span>Canonical URL</span><a href={source.url} target="_blank" rel="noreferrer">{source.url} ↗</a></div><div className="record-facts"><div><span>Observed evidence</span><strong>{source.evidenceCount} citations</strong></div><div><span>Your brand</span><strong><StatusDot tone={!reviewed ? "gray" : source.clientPresent ? "green" : "red"} />{!reviewed ? "Not human-reviewed" : source.clientPresent ? "Present" : "Absent"}</strong></div><div><span>Automated reachability</span><strong>{source.crawlerAccess}</strong></div><div><span>Last human review</span><strong>{source.reviewedAt || (viewer.mode === "demo" ? "Seeded demo" : "Not reviewed")}</strong></div></div><h2>Why this source is in the map</h2><p>It was cited across {source.engines.join(", ") || "the recorded provider set"}. This proves citation recurrence only. Automated retrieval can observe reachability; page-level brand presence and editorial feasibility remain separate human-review tasks.</p></section>
      <aside className="panel"><span className="eyebrow">Candidate route</span><h2>{reviewed ? source.route : "unreviewed"}</h2><ol className="record-steps"><li>Inspect the source and note the automated reachability evidence.</li><li>Verify whether your brand and competitors actually appear.</li><li>Confirm editorial fit and evidence requirements.</li><li>Choose only a legitimate, disclosed contribution route.</li><li>Track publication and later citations independently.</li></ol><Link className="text-link" href="/app/opportunities">Open Opportunities <Arrow /></Link></aside>
    </div>
    <section className="panel source-credibility"><div><span className="eyebrow">Estimated source credibility</span><h2>{credibility.score}/100 · {credibility.confidence} confidence</h2><p>This transparent heuristic uses only Foremention-observed citation recurrence, provider coverage, publisher-type signals, observed reachability, and explicit human review. It is not a third-party domain-authority score.</p></div><div><strong>Signals used</strong><ul>{credibility.signals.map((signal) => <li key={signal}>{signal}</li>)}</ul></div><div><strong>Still unknown</strong><ul>{credibility.missing.map((signal) => <li key={signal}>{signal}</li>)}</ul></div></section>
