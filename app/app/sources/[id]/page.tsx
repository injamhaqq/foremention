...
export default async function SourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!source) notFound();
  const evidence = source.sourceId ? (await loadSourceEvidenceContexts(viewer, [source.sourceId]))[source.sourceId] || [] : [];
  ...
  return <main className="workspace">
    <div className="workspace-heading"><div><span className="eyebrow">Exact source record · {source.id.slice(0, 8).toUpperCase()}</span><h1>{source.domain}</h1><p>{source.title}</p></div><Link className="button button--outline" href="/app/source-map">← Back to Source Map</Link></div>
