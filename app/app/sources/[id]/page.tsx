...
export default async function SourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="workspace">
    ...
    <section className="panel source-snapshot-history">
      <div className="panel-heading"><div><span className="eyebrow">Saved page observations</span><h2>What changed on this cited page?</h2><p>Foremention saves bounded retrieval metadata and a text fingerprint—not the page body. A changed fingerprint records an observed difference; it does not prove what caused the difference.</p></div></div>
      {snapshots.length ? <div className="table-wrap"><table><thead><tr><th>Checked</th><th>Result</th><th>Page</th><th>Evidence link</th></tr></thead><tbody>{snapshots.map((snapshot) => <tr key={snapshot.id}><td>{snapshotDate(snapshot.checkedAt)}</td><td><strong>{snapshotStateLabel(snapshot.changeState)}</strong><div className="table-caption">{snapshot.access}{snapshot.httpStatus ? ` · HTTP ${snapshot.httpStatus}` : ""}{snapshot.changeReason ? ` · ${snapshot.changeReason}` : ""}</div></td><td>{snapshot.pageTitle || source.title}<div className="table-caption">{snapshot.finalUrl}</div></td><td>{snapshot.collectionLinked ? `${snapshot.linkedObservationCount} citation observation${snapshot.linkedObservationCount === 1 ? "" : "s"}` : "Manual page check"}</td></tr>)}</tbody></table></div> : <div className="empty-state"><strong>No saved page observations yet.</strong><span>Run a collection or inspect this source to create the first bounded fingerprint.</span></div>}
    </section>
