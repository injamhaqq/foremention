  ...
  return <main className="print-record">
    ...
    <section className="print-record__answers">
      ...
      {records.map((record) => <article key={record.id}>
        ...
        <footer><span>Owner: {record.ownerId ? "assigned" : "unassigned"}</span><span>Due: {formatDate(record.dueAt)}</span><span>Observed direction: {record.outcomeState.replaceAll("_", " ")}</span><span>Comparison eligible: {record.comparisonEligible === null ? "not assessed" : record.comparisonEligible ? "yes" : "no"}</span></footer>
    ...
    <footer className="print-record__footer">Foremention reports chronology and eligible observed association, not causal attribution. Directional movement is not automatically business value, and operational value is not automatically economic ROI.</footer>
  </main>;
}
