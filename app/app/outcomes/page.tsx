...
export default async function OutcomesPage() {
  ...
  const [evidence, opportunities, followUps] = await Promise.all([
    ...
    opportunityIds.length ? readLedgerTable(supabaseRest<OutcomeLedgerOpportunityRow[]>(
      `opportunities?select=id,owner_id,due_at,next_action,status,updated_at&id=in.(${opportunityIds.join(",")})&organization_id=eq.${context.organizationId}&project_id=eq.${context.projectId}`,
      { token: viewer.accessToken },
    ), pending) : [],
    assetIds.length ? readLedgerTable(supabaseRest<OutcomeLedgerFollowUpRow[]>(
  ...
  return <main className="workspace">
    <div className="workspace-heading">
      <div><span className="eyebrow">Outcome intelligence</span><h1>What changed, what did we do, and what happened afterward?</h1><p>The Outcome Ledger connects reviewed Recommendation Record evidence to human decisions, owned actions, completion, and later measurements without claiming that chronology proves causation.</p></div>
      <div className="workspace-heading__actions"><Link className="button button--outline" href="/app/resolutions">Open Resolution Center</Link><Link className="button button--ink" href="/app/outcomes/print">Board-ready export</Link></div>
    </div>
    ...
    <section className="panel">
      <div className="panel-heading"><div><span className="eyebrow">Executive digest</span><h2>The CMO view, grounded in the ledger.</h2></div><Link href="/app/analytics">Open exact Comparisons →</Link></div>
      <div className="system-grid">
        <article><span>01</span><h3>What changed?</h3><p>{digest.whatChanged}</p></article>
        <article><span>02</span><h3>What needs attention?</h3><p>{digest.needsAttention}</p></article>
        <article><span>03</span><h3>Where are competitors moving?</h3><p>{digest.competitorMovement}</p></article>
        <article><span>04</span><h3>What actions are open?</h3><p>{digest.openActions}</p></article>
        <article><span>05</span><h3>Did an intervention coincide with change?</h3><p>{digest.interventionObservation}</p></article>
        <article><span>06</span><h3>What should we review next?</h3><p>{digest.reviewNext}</p></article>
      </div>
    </section>

    <section className="panel">
      <div className="panel-heading"><div><span className="eyebrow">Business value report</span><h2>Operational value first. Economic attribution only when verified.</h2></div></div>
      <div className="metric-grid metric-grid--compact">
        <article><span>Issues identified</span><strong>{value.issuesIdentified}</strong><small>reviewed baselines attached</small></article>
        <article><span>Actions approved</span><strong>{value.actionsApproved}</strong><small>human decision recorded</small></article>
        <article><span>Actions completed</span><strong>{value.actionsCompleted}</strong><small>application recorded</small></article>
        <article><span>Items remeasured</span><strong>{value.itemsRemeasured}</strong><small>later measurement completed</small></article>
        <article><span>Improvements observed</span><strong>{value.improvementsObserved}</strong><small>eligible directional movement</small></article>
        <article><span>Regressions observed</span><strong>{value.regressionsObserved}</strong><small>eligible directional movement</small></article>
        <article><span>Competitive gaps addressed</span><strong>{value.competitiveGapsAddressed}</strong><small>completed comparison interventions</small></article>
        <article><span>Unresolved items</span><strong>{value.unresolvedItems}</strong><small>completion or eligible outcome still open</small></article>
      </div>
      <div className="inline-notice" role="note"><strong>Economic ROI: not demonstrated.</strong><p>{value.economicValue.basis} Current operational value: {value.operationalValue}</p></div>
    </section>

    <section className="panel">
      <div className="panel-heading"><div><span className="eyebrow">Reporting cadence</span><h2>Weekly, monthly, and quarterly value summaries.</h2></div></div>
      <div className="system-grid">{periods.map((period) => <article key={period.label}><span>{period.days}d</span><h3>{period.label}</h3><p><strong>{period.report.actionsCompleted}</strong> completed · <strong>{period.report.itemsRemeasured}</strong> remeasured · <strong>{period.report.improvementsObserved}</strong> improved · <strong>{period.report.regressionsObserved}</strong> regressed · <strong>{period.report.unresolvedItems}</strong> unresolved</p></article>)}</div>
    </section>

    <div className="metric-grid metric-grid--compact">
      <article><span>Ledger records</span><strong>{records.length}</strong><small>reviewed intervention chains</small></article>
      <article><span>Open actions</span><strong>{openActions}</strong><small>approved, not completed</small></article>
      <article><span>Eligible outcomes</span><strong>{eligibleOutcomes}</strong><small>exact comparison gate passed</small></article>
      <article><span>Incomparable measurements</span><strong>{value.incomparableMeasurements}</strong><small>retained, outcome comparison withheld</small></article>
    </div>
    ...
    <section className="panel panel--flush">
      {records.length ? <div className="outcome-ledger">{records.map((record) => <article className="outcome-ledger__record" key={record.id}>
        ...
        {record.comparison ? <div className="outcome-ledger__comparison">
          <div className="panel-heading"><div><span className="eyebrow">Eligible observed outcome</span><h3>{outcomeLabel(record.outcomeState)}</h3></div></div>
      ...
      </article>)}</div> : <div className="empty-state"><h2>No outcome records yet.</h2><p>This ledger fills in as reviewed Recommendation Record evidence becomes a recommendation, a human decision, an owned action, a completed intervention, and an eligible later measurement.</p><Link className="button button--ink" href="/app/resolutions">Start in Resolution Center →</Link></div>}
    </section>
