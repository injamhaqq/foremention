...
const outcomeLabel = (value: string) => ({
  higher_observed: "Higher direction observed",
  lower_observed: "Lower direction observed",
  mixed_observed: "Mixed direction observed",
  no_directional_change: "No directional change",
...
export default async function OutcomesPage() {
  ...
  if (viewer.mode === "demo") {
    return <main className="workspace">
      <div className="workspace-heading"><div><span className="eyebrow">Outcome intelligence</span><h1>Outcome Ledger</h1><p>Observation → evidence → recommendation → decision → action → owner → completion → later measurement → observed direction.</p></div><Link className="button button--outline" href="/app/resolutions">Open Resolution Center</Link></div>
  ...
  return <main className="workspace">
    ...
    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Reporting cadence</span><h2>Weekly, monthly, and quarterly value summaries.</h2></div></div><div className="system-grid">{periods.map((period) => <article key={period.label}><span>{period.days}d</span><h3>{period.label}</h3><p><strong>{period.report.actionsCompleted}</strong> completed · <strong>{period.report.itemsRemeasured}</strong> remeasured · <strong>{period.report.higherObserved}</strong> higher direction · <strong>{period.report.lowerObserved}</strong> lower direction · <strong>{period.report.unresolvedItems}</strong> unresolved</p></article>)}</div></section>

    <div className="metric-grid metric-grid--compact"><article><span>Ledger records</span><strong>{records.length}</strong><small>reviewed intervention chains</small></article><article><span>Open actions</span><strong>{openActions}</strong><small>approved, not completed</small></article><article><span>Eligible comparisons</span><strong>{eligibleComparisons}</strong><small>exact comparison gate passed</small></article><article><span>Incomparable measurements</span><strong>{value.incomparableMeasurements}</strong><small>retained, directional comparison withheld</small></article></div>
