...
test("activation guidance advances one evidence-backed step at a time", () => {
  ...
  assert.equal(deriveActivationStage({ ...base, workspaceConfigured: true, approvedQuestions: 5, firstCollectionCompleted: true, firstRecordReviewed: true, firstActionCreated: true, comparableReviewedCycles: 1 }).key, "second_comparable_cycle");
  assert.equal(deriveActivationStage({ ...base, workspaceConfigured: true, approvedQuestions: 5, firstCollectionCompleted: true, firstRecordReviewed: true, firstActionCreated: true, comparableReviewedCycles: 2 }).key, "retained_loop");
});

test("a run or action cannot skip the required human-review boundary", () => {
  const stage = deriveActivationStage({ ...base, workspaceConfigured: true, approvedQuestions: 5, firstCollectionCompleted: true, firstActionCreated: true, comparableReviewedCycles: 2 });
