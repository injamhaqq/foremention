...
test("authenticated light semantic status pills force a dark canonical foreground", () => {
  assert.match(css, /\.app-frame\s+:where\(\.pill--open,\s*\.pill--partial,\s*\.feasibility--high,\s*\.feasibility--medium\)[\s\S]{0,120}color:\s*var\(--fm-bg\)/);
...
test("blocked and low light semantic pills retain an explicit readable foreground", () => {
  assert.match(css, /\.app-frame\s+:where\(\.pill--blocked,\s*\.feasibility--low\)[\s\S]{0,120}color:\s*#5f2f2a/i);
