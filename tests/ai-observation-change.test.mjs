...
test("AI Observation Change Graph defensively withholds an incomparable pair", () => {
  ...
  assert.match(missingModel.note, /missing exact model provenance/i);

  const changedMatrix = buildAiObservationChangeGraph({
    latest: { id: "latest", methodologyVersion: "3.0" },
    previous: { id: "previous", methodologyVersion: "3.0" },
    answers: [answer({ runId: "previous" }), answer({ runId: "latest", promptKey: "q2" })],
  });
  assert.equal(changedMatrix.status, "withheld");
  assert.match(changedMatrix.note, /matrix changed/i);
});
...
test("Analytics preserves exact comparable-run gating and separates AI changes from Source Change Graph", async () => {
