...
test("canonical strategic pages expose curated Markdown mirrors and content negotiation", async () => {
  ...
  for (const slug of ["product", "methodology"]) {
    ...
    assert.ok(page.includes(`markdownPath: "/${slug}.md"`));
    ...
    assert.ok(headers.includes(`<https://foremention.com/${slug}>; rel="canonical"`));
  ...
  assert.ok(worker.includes('response.headers.set("Content-Type", "text/markdown; charset=utf-8")'));
  assert.ok(worker.includes('return complete(response)'));
