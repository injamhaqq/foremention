...
test("pricing comparison uses dated, first-party plan facts and honest activation language", async () => {
  ...
  assert.match(pricing, /August 2, 2026/);
  assert.match(pricing, /Peec AI[\s\S]*Starter[\s\S]*\$95 \/ month[\s\S]*https:\/\/peec\.ai\/ai-instructions/);
  assert.match(pricing, /Scrunch[\s\S]*Core[\s\S]*\$250 \/ month[\s\S]*https:\/\/scrunch\.com\/pricing\//);
