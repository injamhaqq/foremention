...
test("large workspace lists use bounded pages or incremental rendering", async () => {
  ...
  const opportunities = await readFile(new URL("../components/opportunity-list.tsx", import.meta.url), "utf8");
  ...
  assert.match(opportunities, /visibleCount/);
  assert.match(opportunities, /Load 10 more opportunities/);
