...
test("public prompt checker exposes one real Browser Run grounded answer with fetched citations", () => {
  ...
  assert.match(promptCheck, /provider: "Cloudflare Workers AI \+ Browser Run"/);
  ...
  assert.doesNotMatch(promptCheck, /GEMINI_API_KEY|api\.groq\.com|s\.jina\.ai/);
});
