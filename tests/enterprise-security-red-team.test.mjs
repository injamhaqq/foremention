...
const governanceMigration = "supabase/migrations/20260830000300_enterprise_security_governance.sql";
const auditHardeningMigration = "supabase/migrations/20260830000400_audit_log_hardening.sql";
...
test("role migration order makes admin valid before any policy can use it", async () => {
  const retention = await read("supabase/migrations/20260829000100_retention_loop_v1.sql");
  const adminAlter = retention.indexOf("add value if not exists 'admin'");
  const transaction = retention.indexOf("begin;");
  const firstAdminPolicyUse = retention.indexOf("'owner','admin'");
  assert.ok(adminAlter >= 0, "admin enum value must be added");
  assert.ok(transaction > adminAlter, "admin enum must be added before the explicit migration transaction");
  assert.ok(firstAdminPolicyUse > transaction, "admin policy use must happen after enum creation");
});
