...
test("commercial pipeline preserves the canonical lifecycle and closed deals cannot silently reopen", () => {
  ...
  assert.equal(canTransitionCommercialStage("demo", "pilot_proposed"), true);
  assert.equal(canTransitionCommercialStage("pilot_active", "won"), true);
...
test("commercial migration extends the existing founder-internal CRM and adds auditable truth stores", async () => {
  const migration = await readFile(new URL("../supabase/migrations/20260830000400_commercial_engine.sql", import.meta.url), "utf8");

  for (const table of ["commercial_accounts", "commercial_contacts", "commercial_opportunities", "commercial_events"]) {
    assert.match(migration, new RegExp(`alter table public\\.${table}`, "i"));
    assert.doesNotMatch(migration, new RegExp(`create table if not exists public\\.${table}`, "i"));
    assert.match(migration, new RegExp(`alter table public\\.${table} enable row level security`, "i"));
  }

  for (const table of ["commercial_stage_events", "pricing_research_records", "commercial_metric_periods"]) {
    assert.match(migration, new RegExp(`create table if not exists public\\.${table}`, "i"));
    assert.match(migration, new RegExp(`alter table public\\.${table} enable row level security`, "i"));
  }

  assert.doesNotMatch(migration, /create table if not exists public\.commercial_activities/i);
  ...
  assert.match(migration, /revoke all on public\.commercial_accounts from anon, authenticated/i);
  assert.match(migration, /revoke all on public\.pricing_research_records from public, anon, authenticated/i);
