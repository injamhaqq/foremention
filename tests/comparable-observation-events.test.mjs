...
test("safe comparable event layer separates presence, citation observations, unique sources, and answer text", () => {
...
test("event layer returns no movement unless safe intelligence exposes an exact prior pair", () => {
...
test("unreviewed and excluded rows never enter comparable answer-slot events", () => {
  const events = buildComparableObservationEvents({
...
test("customer event layer delegates comparability and never exposes provider economics", async () => {
  ...
  assert.match(source, /already-selected safe exact comparable pair/i);
  assert.match(source, /loadSafeWeeklyIntelligence/);
  assert.match(source, /never chooses which runs are comparable/i);
  assert.match(analytics, /loadSafeWeeklyIntelligence/);
});
