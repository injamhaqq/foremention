...
export function SourceMapTable({ entries, canEdit, demo }: { entries: SourceMapEntry[]; canEdit: boolean; demo: boolean }) {
  ...
  return <>
    <div className="table-tools table-tools--source-map">
      ...
      <label><span>Sort by</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value as typeof sortBy)}><option value="observations">Citation observations</option><option value="domain">Domain</option><option value="review-status">Review status (human)</option></select></label>
