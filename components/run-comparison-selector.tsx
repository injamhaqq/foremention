...
export function RunComparisonSelector({ runs }: { runs: VisibilityRun[] }) {
  ...
  return <form className="run-comparison-selector" action="/app/runs/compare" method="get">
    <div><span className="eyebrow">Run inspection</span><strong>Inspect two reviewed collections.</strong><small>Movement appears only when Foremention can prove the same methodology and exact persisted buyer-question, provider, and model matrix.</small></div>
    ...
    <button className="button button--ink" type="submit" disabled={completed.length < 2}>Inspect runs</button>
    {completed.length < 2 && <small>Two completed or partial reviewed runs are required.</small>}
  </form>;
}
