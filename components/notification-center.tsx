...
export function NotificationCenter({ initialItems, demo }: { initialItems: WorkspaceNotification[]; demo: boolean }) {
  ...
  return <section className="panel panel--flush">
    ...
    {items.length ? <div className="notification-list">{items.map((item) => <article className={item.read ? "" : "is-unread"} key={item.id}><div><span>{item.kind.replaceAll("_", " ")}{item.count > 1 ? ` · ${item.count} similar events` : ""}</span><strong>{item.title}</strong><p>{item.body}</p><small>Most recent: {item.createdAt}</small></div>{item.href && <Link href={item.href}>Open →</Link>}</article>)}</div> : <div className="empty-state"><h2>No operational alerts yet.</h2><p>Run completion, failures, reviewed Source Maps, and workspace access changes will appear here from real customer events.</p><Link className="button button--ink" href="/app/runs">Start with Answer Runs →</Link></div>}
