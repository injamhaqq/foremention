...
const assetTypes = [
  ["comparison_page", "Comparison brief"],
...
function completedSteps(record: ResolutionRecord) {
  return [
    ...
    record.followUp.status === "complete",
  ];
}
...
async function readResponse(response: Response) {
  ...
  try {
    ...
    throw new Error(response.ok ? "The server returned an unreadable response. Try again." : `The request failed (${response.status}) without a readable explanation.`);
  }
}
...
function LoopProgress({ record }: { record: ResolutionRecord }) {
  ...
  return <ol className={styles.loop} aria-label="Resolution progress">
    {loopSteps.map((step, index) => {
      ...
      return <li className={styles[state]} key={step} aria-current={state === "current" ? "step" : undefined}>
        <span>{complete[index] ? "✓" : String(index + 1).padStart(2, "0")}</span>
        <strong>{step}</strong>
      </li>;
    })}
  </ol>;
...
function EvidenceList({ items }: { items: ResolutionEvidence[] }) {
  if (!items.length) return <div className={styles.inlineEmpty}>
    <strong>No reviewable evidence is attached.</strong>
    <p>This problem cannot produce an approval-ready asset until it is connected to a dated answer, citation, page review, or recorded workspace observation.</p>
  </div>;
  ...
  return <div className={styles.evidenceList}>
    {items.map((item) => {
      ...
      return <article key={item.id}>
        <div className={styles.cardMeta}><span>{readable(item.kind)}</span><time>{formatDate(item.observedAt)}</time></div>
        <h4>{item.title}</h4>
        <p>{item.detail}</p>
        {href && (href.startsWith("/")
          ? <Link className={styles.inlineLink} href={href}>Inspect recorded evidence →</Link>
          : <a className={styles.inlineLink} href={href} target="_blank" rel="noreferrer">Open recorded source ↗</a>)}
      </article>;
    })}
  </div>;
}
...
function EmptyState({ demo }: { demo: boolean }) {
  return <section className={styles.emptyState}>
    <span className="eyebrow">No measured problem selected</span>
    <h2>{demo ? "Resolution actions are not available in the fictional demo." : "Measure and review evidence before proposing a fix."}</h2>
    <p>{demo ? "The demo stays read-only and cannot create customer approval records, applied locations, or follow-up runs." : "Resolution Center starts with an observed problem from your own reviewed workspace records. It never manufactures a problem, solution, or result to make the page look populated."}</p>
    <div className={styles.buttonRow}>
      <Link className="button button--ink" href="/app/runs">Review answer runs</Link>
      <Link className="button button--outline" href="/app/opportunities">Inspect priority gaps</Link>
    </div>
  </section>;
}
...
export function ResolutionCenter({ demo, role }: { demo: boolean; role: WorkspaceRole }) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    if (demo) return;
    try {
      const response = await fetch("/api/resolutions", { signal, headers: { accept: "application/json" } });
      const payload = await readResponse(response);
      const data = payload.data as { resolutions?: ResolutionRecord[] } | undefined;
      if (!response.ok) throw new Error(typeof payload.error === "string" ? payload.error : "Resolution records could not be loaded.");
      const next = Array.isArray(data?.resolutions) ? data.resolutions : [];
  ...
  const mutate = useCallback(async (method: "POST" | "PATCH", body: Record<string, unknown>, label: string, success: string) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "The resolution could not be updated.");
    } finally {
      setBusy("");
    }
  }, [busy, canWrite, load]);
  ...
  async function saveDraft() {
    if (!active || !draft.title.trim() || !draft.summary.trim() || !draft.content.trim() || !draft.limitations.trim()) {
      setError("Complete the title, summary, asset content, and limitations before saving the draft.");
      return;
    }
  ...
  async function decide(decision: ApprovalStatus) {
    ...
    if (decision === "changes_requested" && !approvalNote.trim()) {
      setError("Describe the requested change so the next revision has an accountable instruction.");
      return;
    }
  ...
  async function requestRemeasurement() {
    ...
    setNotice("");
    try {
      const requestResponse = await fetch("/api/resolutions", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ action: "remeasure", resolutionId: active.id }),
      });
      ...
      if (!runRequest?.measurementId || !runRequest.promptIds?.length || !runRequest.providers?.length) {
        throw new Error("The follow-up request was saved, but its comparable run details are incomplete. No provider call was started.");
      }
      ...
      const runResponse = await fetch("/api/runs", {
        ...
        headers: {
          ...
          "idempotency-key": runRequest.idempotencyKey || `resolution:${active.id}:measurement:${runRequest.measurementId}`,
        },
        body: JSON.stringify({ promptIds: runRequest.promptIds, providers: runRequest.providers }),
      });
      const runPayload = await readResponse(runResponse);
      if (!runResponse.ok || typeof runPayload.id !== "string") {
        throw new Error(`${typeof runPayload.error === "string" ? runPayload.error : "The comparable run could not be queued."} The durable follow-up request remains available to retry.`);
      }

      const attachResponse = await fetch("/api/resolutions", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ action: "remeasure", resolutionId: active.id, rerunId: runPayload.id, measurementId: runRequest.measurementId }),
      });
      const attachPayload = await readResponse(attachResponse);
      if (!attachResponse.ok) {
        throw new Error(`${typeof attachPayload.error === "string" ? attachPayload.error : "The queued run could not be attached to the resolution."} The run and durable follow-up request were preserved for recovery.`);
      }
      await load();
      setNotice("Comparable follow-up measurement queued. Its result will remain separate from the baseline until review.");
    } catch (caught) {
      await load().catch(() => undefined);
      setError(caught instanceof Error ? caught.message : "The follow-up measurement could not be queued. Its durable request remains available to retry.");
    } finally {
      setBusy("");
    }
  }

  if (loading) return <section className={styles.clientLoading} aria-busy="true">
    <span className={styles.pulse} />
    <h2>Loading measured problems…</h2>
    <p>Foremention is retrieving this workspace’s reviewed evidence and existing approval records.</p>
  </section>;

  if (!active) return <>
    {error && <div className={styles.errorBanner} role="alert"><strong>Resolution Center is unavailable.</strong><span>{error}</span><button type="button" onClick={() => { setLoading(true); void load(); }}>Try again</button></div>}
  ...
  return <div className={styles.center}>
    ...
    {role === "viewer" && <div className={styles.noticeBanner} role="status">
      <span>This workspace is read-only for viewers. An owner, admin, or analyst can create and edit resolution work.</span>
    </div>}

    <div className={styles.centerGrid}>
      <aside className={styles.problemRail} aria-label="Measured problems">
        <div className={styles.railHeading}>
          <span>Measured problems</span>
          <strong>{records.length}</strong>
        </div>
        <div className={styles.problemList}>
          {records.map((record) => <button className={record.id === active.id ? styles.selectedProblem : ""} type="button" key={record.id} onClick={() => hydrateEditor(record)}>
            <span>{readable(record.problem.type)}</span>
            <strong>{record.problem.title}</strong>
            <small>{readable(record.status)}</small>
          </button>)}
        </div>
      ...
      <div className={styles.detail}>
        <section className={styles.problemPanel}>
          <div className={styles.sectionHeading}>
            <div><span>01 · Specific problem</span><h2>{active.problem.title}</h2></div>
            <div className={styles.confidence}><small>Recorded confidence</small><strong>{readable(active.problem.confidence)}</strong></div>
          </div>
          <p className={styles.problemSummary}>{active.problem.summary}</p>
          <dl className={styles.factStrip}>
            <div><dt>Problem type</dt><dd>{readable(active.problem.type)}</dd></div>
            <div><dt>Observed</dt><dd>{formatDate(active.problem.observedAt)}</dd></div>
            <div><dt>Resolution state</dt><dd>{readable(active.status)}</dd></div>
          </dl>
        </section>

        <section className={styles.sectionPanel}>
          <div className={styles.sectionHeading}>
            <div><span>02 · Inspectable evidence</span><h3>What supports this problem?</h3></div>
            <small>{active.evidence.length} recorded item{active.evidence.length === 1 ? "" : "s"}</small>
          </div>
        ...
        <section className={styles.sectionPanel}>
          ...
          <div className={styles.editorTabs} role="tablist" aria-label="Solution asset editor">
        ...
        <section className={styles.twoColumn}>
          <div className={styles.sectionPanel}>
            <div className={styles.sectionHeading}><div><span>04 · Customer decision</span><h3>Approve, reject, or request changes.</h3></div><small>{readable(active.approval.status)}</small></div>
            <label className={styles.stackLabel}>Decision note<textarea rows={4} maxLength={2_000} value={approvalNote} onChange={(event) => setApprovalNote(event.target.value)} placeholder="Record why this is approved, rejected, or needs revision." /></label>
            <div className={styles.decisionButtons}>
              {active.status === "draft" && <button className="button button--outline" type="button" disabled={!canWrite || busy !== "" || !proposalSaved} onClick={() => void submitForReview()}>{busy === "decision:submit" ? "Submitting…" : "Submit for review"}</button>}
              <button className="button button--ink" type="button" disabled={!canManage || busy !== "" || !proposalSaved} onClick={() => void decide("approved")}>{busy === "decision:approved" ? "Recording…" : "Approve asset"}</button>
              <button className="button button--outline" type="button" disabled={!canManage || busy !== "" || !proposalSaved} onClick={() => void decide("changes_requested")}>{busy === "decision:changes_requested" ? "Recording…" : "Request changes"}</button>
              <button className={styles.textButton} type="button" disabled={!canManage || busy !== "" || !proposalSaved} onClick={() => void decide("rejected")}>{busy === "decision:rejected" ? "Recording…" : "Reject"}</button>
            </div>
            {!canManage && role !== "viewer" && <p className={styles.auditLine}>An owner or admin must record the approval decision.</p>}
            {active.approval.decidedAt && <p className={styles.auditLine}>Decision recorded {formatDate(active.approval.decidedAt)}{active.approval.decidedBy ? ` by ${active.approval.decidedBy}` : ""}.</p>}
          </div>
        ...
        <section className={`${styles.sectionPanel} ${styles.followUp}`}>
          <div>
            ...
            {active.followUp.summary && <blockquote>{active.followUp.summary}</blockquote>}
          </div>
