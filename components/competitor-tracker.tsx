...
export function CompetitorTracker({ initial, canManage, demo }: { initial: CompetitorTracking[]; canManage: boolean; demo: boolean }) {
  ...
  return <>
    ...
    {initial.length ? <div className="competitor-grid">{initial.map((item) => <article className="panel" key={item.id}><div className="competitor-card__head"><div><span className="eyebrow">{item.type}</span><h2>{item.name}</h2>{item.website && <a href={item.website} target="_blank" rel="noreferrer">Website ↗</a>}</div><span className={`status-chip ${item.active ? "status-chip--active" : ""}`}>{item.active ? "Active" : "Paused"}</span></div><dl><div><dt>Observed in AI answers</dt><dd>{item.totalAnswers ? `${item.answerMentions} of ${item.totalAnswers}` : "No answers yet"}</dd></div><div><dt>Reviewed cited pages</dt><dd>{item.reviewedCitationPages}</dd></div><div><dt>Sources shared with your brand</dt><dd>{item.sourceOverlap}</dd></div><div><dt>Change from the previous collection</dt><dd>{item.trendDelta === null ? "Needs 2 collections" : `${item.trendDelta > 0 ? "+" : ""}${item.trendDelta} percentage points`}</dd></div></dl><small>{item.mentionFrequencyPct === null ? "No completed answer baseline yet." : `${item.name} appeared in ${item.answerMentions} of ${item.totalAnswers} recorded answers (${item.mentionFrequencyPct}%).`} These are exact-name observations from persisted answers and human-reviewed cited pages. This does not estimate market share.</small><button type="button" disabled={!canManage || demo || updating === item.id} onClick={() => void toggle(item)}>{updating === item.id ? "Updating…" : item.active ? "Pause tracking" : "Resume tracking"}</button></article>)}</div> : <div className="empty-state"><h2>No competitors tracked yet.</h2><p>Add only brands a real buyer would compare. Foremention will measure their observed answer mentions and reviewed source overlap after collections run.</p></div>}
  </>;
}
