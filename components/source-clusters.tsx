...
export function SourceClusters({ clusters }: { clusters: SourceCluster[] }) {
  ...
  return <section className="source-clusters" aria-labelledby="source-clusters-title">
    ...
    <div>{clusters.map((cluster) => <article key={cluster.id}><span>{cluster.label}</span><strong>{cluster.sourceCount} source{cluster.sourceCount === 1 ? "" : "s"}</strong><small>{cluster.observations} citation observation{cluster.observations === 1 ? "" : "s"} · {cluster.domains.length} domain{cluster.domains.length === 1 ? "" : "s"}</small>{cluster.entries[0] && <Link href={`/app/sources/${cluster.entries[0].id}`}>Open leading record →</Link>}</article>)}</div>
  </section>;
}
