...
export function OpportunityList({ rows, demo }: { rows: RankedSource[]; demo: boolean }) {
  ...
  return <>{message && <p className="inline-notice" role="status">{message}</p>}<div className="opportunity-list">{visible.map((source,index) => <article key={source.id}><div className={`opportunity-score ${source.score === null ? "opportunity-score--review" : ""}`}><span>{source.score === null ? "Evidence" : "Reviewed priority"}</span>{source.score === null ? <strong>Review</strong> : <><strong>{source.score}</strong><small>/100</small></>}</div><div><span className="opportunity-rank">#{index+1} · {source.type} · {source.score === null ? "verification required" : "confirmed gap"}</span><h2>{source.domain}</h2><p>{source.title}</p>{source.evidence && <div className="opportunity-evidence"><strong>Observed for: {source.evidence.prompt}</strong><span>{source.evidence.provider}{source.evidence.model ? ` · ${source.evidence.model}` : ""}{source.evidence.citationOrdinal ? ` · citation ${source.evidence.citationOrdinal}` : ""} · {source.evidence.observedAt}</span></div>}<div className="opportunity-meta"><span>{source.evidenceCount} citation observation{source.evidenceCount === 1 ? "" : "s"}</span><span>{source.engines.length} provider{source.engines.length === 1 ? "" : "s"}</span>{source.score !== null && <><span>{source.influence} influence</span><span>{source.feasibility} feasibility</span><span>{source.route}</span></>}</div></div><div className="opportunity-actions"><a href={`/app/sources/${source.id}`}>Inspect evidence <Arrow /></a><button type="button" disabled={busy === source.id || (!source.sourceId && !demo) || source.score === null} onClick={() => void track(source)}>{busy === source.id ? "Adding…" : source.score === null ? "Verify before tracking" : "Track action"}</button></div></article>)}</div>{rows.length > 6 && <div className="opportunity-more"><button className="button button--outline" type="button" onClick={() => setShowAll((current) => !current)}>{showAll ? "Show first 6" : `Show all ${rows.length} sources`}</button><p>{showAll ? "All observed sources are visible." : `${rows.length - 6} more sources remain in the review queue.`}</p></div>}</>;
}
