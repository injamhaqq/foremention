...
export function OpportunityList({ rows, demo }: { rows: RankedSource[]; demo: boolean }) {
  ...
  return <>
    ...
    <div className="opportunity-list">{visible.map((source,index) => <article data-workspace-item tabIndex={-1} key={source.id}>
      ...
      <div className="opportunity-actions"><a data-workspace-review href={`/app/sources/${source.id}`}>Inspect evidence <Arrow /></a><button data-workspace-action type="button" disabled={busy === source.id || (!source.sourceId && !demo) || source.score === null} onClick={() => void track(source)}>{busy === source.id ? "Adding…" : source.score === null ? "Verify before tracking" : "Create action"}</button><CommentThread entityType="priority_gap" entityId={source.id} demo={demo} /></div>
    </article>)}</div>
