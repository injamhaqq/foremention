...
export function EvidenceManager({ initialItems, demo }: { initialItems: WorkspaceEvidence[]; demo: boolean }) {
  ...
  return <div>
    ...
    {items.length ? <div className="evidence-table"><div className="evidence-row evidence-row--head"><span>Evidence</span><span>Source / rights</span><span>Status</span><span>Verified</span></div>{items.map((item) => <div className="evidence-row" key={item.id}><div><strong>{item.title}</strong><small>{item.type}</small></div><div>{item.sourceUrl ? <a href={item.sourceUrl} target="_blank" rel="noreferrer">Open source ↗</a> : <span>No URL supplied</span>}<small>{item.rights || "Rights not recorded"}</small></div><span className={`status-chip status-chip--${item.status === "verified" ? "active" : ""}`}>{item.status}</span><span>{item.verifiedAt || "Not verified"}</span></div>)}</div> : <div className="empty-state"><h2>No workspace evidence yet.</h2><p>Add the first dated proof above. Unverified items never become public claims automatically.</p></div>}
  </div>;
}
